...
//                NSString* text = _labelForValue(minBound + (maxBound - minBound) / _verticalGridStep * (i + 1));
                NSString *text = [NSString stringWithFormat:@"%.1f",minBound + (maxBound - minBound) / _verticalGridStep * (i + 1)];
                if(!text)
                    continue;
                ...
                [text
                 ...
                 context:nil]
                .size.width;
                NSLog(@"x : %f",p.x - width - 6);
                UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(p.x - width - 293, p.y - 7, width + 2, 14)];
                label.text = text;
...
- (void)strokeDataPoints
{
    ...
    for(int i=0;i<_data.count;i++) {
        ...
        CATextLayer *pointLabel = [[CATextLayer alloc]init];
        [pointLabel setFont:@"HelveticaNeue"];
        [pointLabel setFontSize:8];
        NSString *pointVal = [NSString stringWithFormat:@"%@",_data[i]];
        
        [pointLabel setFrame:CGRectMake(p.x - 10, p.y - 15, 15, 10)];
        if (![pointVal isEqualToString:tempPointVal]) {
            [pointLabel setString:pointVal];
        }
        tempPointVal = pointVal;
        [pointLabel setAlignmentMode:kCAAlignmentCenter];
        [pointLabel setForegroundColor:[[UIColor darkGrayColor] CGColor]];
        pointLabel.contentsScale = [[UIScreen mainScreen] scale];
        if (self.showPointLabel) {
            [self.layer addSublayer:pointLabel];
        }
...
{
    _showPointLabel = NO;
    _showlabelForValue = NO;
    _color = [UIColor fsDarkGray];
    ...
    _valueLabelFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:10];

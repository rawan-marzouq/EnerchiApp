...
#define GRID_LINES_VERT 30
    ...
    glLoadIdentity();
    // MAX VALUE
    if ([[pointsArray objectAtIndex:[pointsArray count]-1] floatValue] / 1000 > maxVal) {
        maxVal = [[pointsArray objectAtIndex:[pointsArray count]-1] floatValue];
    }
    glOrthof(-1.0f, 1.0f, -3.5f, maxVal, -1.0f, 1.0f); // height
...
//	glDrawArrays(GL_POINTS, MAX_CURVE_POINT_NO-1, 1);
	...
	glLineWidth(1.0);
	glPointSize(1.0);
	glColor4f(118.0/255.0, 206.0/255.0, 94.0/255.0, 1.0);
    glDrawArrays(GL_LINE_STRIP, 0, MAX_CURVE_POINT_NO);
	glPointSize(5.0);
	glDrawArrays(GL_POINTS, MAX_CURVE_POINT_NO-1, 1);
    ...
    //    float test = ((rand()%32768)/32768.0)*0.4-0.2;
    //    [pointsArray addObject:[NSString stringWithFormat:@"%f",test]];
    
    if (self.allPointsArray.count > 0) {
        if (index < self.allPointsArray.count) {
            [pointsArray addObject:[self.allPointsArray objectAtIndex:index]];
//            [pointsArray addObject:[NSString stringWithFormat:@"5000"]];
        }
        
    }
    
//	curve[ curveStart ] =   ((rand()%32768)/32768.0)*0.4-0.2;
    curve[ curveStart ] =   [[pointsArray objectAtIndex:[pointsArray count]-1] floatValue] / 1000;
    NSLog(@"curve[ curveStart ]: %f, Value: %f",curve[ curveStart ], [[pointsArray objectAtIndex:[pointsArray count]-1] floatValue]);
    

    NSLog(@"self.allPointsArray.count: %lu",(unsigned long)self.allPointsArray.count);
...
- (void)startAnimation {
    ...
    pointsArray = [[NSMutableArray alloc]initWithObjects:@"0.0", nil];
    index = 0;
